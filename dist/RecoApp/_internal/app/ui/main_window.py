...
from PyQt6.QtGui import QIcon, QActionGroup, QPixmap, QPalette, QColor
from PyQt6.QtWidgets import QMainWindow, QPushButton, QMessageBox, QDialog, QGridLayout, QWidget, QVBoxLayout, \
    QHBoxLayout, QLabel, QSizePolicy, QFrame, QInputDialog, QToolButton, QLineEdit
...
class MainWindow(QMainWindow):
    ...
    def _add_eye_button(self, line_edit):
        btn = QToolButton(line_edit)
        btn.setIcon(QIcon(icon_path("eye_on.svg")))
        btn.setCursor(Qt.CursorShape.PointingHandCursor)
        btn.setStyleSheet("border: none; padding: 0;")
        btn.setIconSize(btn.sizeHint())
        btn.clicked.connect(lambda: self._toggle_password_eye(line_edit, btn))

        line_edit.setEchoMode(line_edit.EchoMode.Password)
        line_edit.setTextMargins(0, 0, 30, 0)

        # корректное сохранение родного resizeEvent
        original_resize = line_edit.resizeEvent

        def new_resize(event):
            btn.setGeometry(
                line_edit.width() - 28,
                (line_edit.height() - 20) // 2,
                20,
                20
            )
    ...
    def _toggle_password_eye(self, line_edit, button):
        if line_edit.echoMode() == line_edit.EchoMode.Password:
            line_edit.setEchoMode(line_edit.EchoMode.Normal)
            button.setIcon(QIcon(icon_path("eye_off.svg")))
        else:
            line_edit.setEchoMode(line_edit.EchoMode.Password)
            button.setIcon(QIcon(icon_path("eye_on.svg")))
    ...
    def _delete_book_dialog(self):
        # --- Русский диалог ввода названия ---
        dlg = QInputDialog(self)
        dlg.setWindowTitle("Удалить книгу")
        dlg.setLabelText("Введите название книги:")
        ...
        rows = fetch_books(title_like=title, limit=20)
        same = [
            r for r in rows
        ...
        if not same:
            FancyMessageBox.warning(self, "Ошибка", f"Книга «{title}» не найдена.")
            return

        # Собираем авторов
        authors = sorted({(r.get("author") or "").strip() for r in same})
        ...
        if len(authors) > 1:
            ...
            dlg2.setWindowTitle("Уточнение автора")
            dlg2.setLabelText(f"Найдено несколько книг «{title}».\nВыберите автора:")
            dlg2.setComboBoxItems(authors)
            dlg2.setOkButtonText("ОК")
            dlg2.setCancelButtonText("Отмена")

            if dlg2.exec() != QDialog.DialogCode.Accepted:
                return

            author = dlg2.textValue().strip()
            if not author:
                return
        else:
            author = authors[0]
        ...
        # --- удаляем ---
        try:
            deleted = delete_book(title, author=author)
            self.load_catalog(None)
            self._refresh_admin_dashboard()
            FancyMessageBox.information(self, "Удалено", f"Удалено книг: {deleted}")
        except ValueError as e:
            FancyMessageBox.warning(self, "Ошибка", str(e))
        except Exception as e:
            FancyMessageBox.warning(self, "Ошибка", f"Не удалось удалить книгу: {e}")
    ...
    def load_catalog(self, tag = None, q = None):
        ...
        print(f"[DEBUG] load_catalog: tag={tag!r}, q={q!r}, count={len(books)}")
    ...
    def _init_state(self):
        ...
        self.flow = FlowLayout(self.cardsContainer, margin=8, spacing=8)
        ...
        self._add_eye_button(self.lePassword)
        self._add_eye_button(self.leRegPassword)
        self._add_eye_button(self.leRegConfirm)
    ...
    def _setup_toolbar_icons(self):
        self.toolBarMain.setIconSize(QtCore.QSize(60, 60))
